...
export function sendNotification(link: Link, store: Store) {
  ...
  if (notifications.python.interpreter && notifications.python.script_path) {
    Logger.debug("↗ running Python script");
    sendToPython(link, store);
  }
  ...
  if (
    ...
    notifications.discord.webHookUrl.length > 0
  ) {
